...
class Attribute:
    ...
    def argument(self, ctx: "mypy.plugin.ClassDefContext") -> Argument:
        ...
        if self.converter.name:
            ...
            if converter and isinstance(converter.node, TypeInfo):
                from mypy.checkmember import (  # To avoid import cycle.
                    type_object_type,
                )

                converter_type = type_object_type(
                    converter.node, ctx.api.builtin_type
                )
            ...
            if (
                isinstance(converter_type, CallableType)
            ...
            if not init_type:
                ctx.api.fail(
                    "Cannot determine __init__ type from converter",
                    self.context,
                )
    ...
    def serialize(self) -> JsonDict:
        ...
        return {
            ...
            "init_type": self.init_type.serialize()
            if self.init_type
            else None,
        }
    ...
    def deserialize(
        ...
        init_type = (
            deserialize_and_fixup_type(raw_init_type, api)
            if raw_init_type
            else None
        )
...
def attr_class_maker_callback(
    ...
    if ctx.api.options.python_version[0] < 3:
        if auto_attribs:
            ctx.api.fail(
                "auto_attribs is not supported in Python 2", ctx.reason
            )
            return
...
def _get_frozen(
    ...
    for super_info in ctx.cls.info.mro[1:-1]:
        if (
            "attrs" in super_info.metadata
            and super_info.metadata["attrs"]["frozen"]
        ):
            return True
    return False
...
def is_valid_overloaded_converter(defn: OverloadedFuncDef) -> bool:
    return all(
        (
            not isinstance(item, Decorator)
            or isinstance(item.func.type, FunctionLike)
        )
...
def _add_init(
    ...
    if all(
        ...
        getattr(arg.variable.type, "type_of_any", None)
